...
import { createHoaSurvey } from "@/lib/server/hoa";
import { hoaSurveyCreateSchema } from "@/lib/validation/hoa";
...
export async function POST(request: Request, context: RouteContext): Promise<NextResponse> {
  ...
  const parsed = hoaSurveyCreateSchema.safeParse(body.value);
  if (!parsed.success) return validationErrorResponse(fieldErrors(parsed.error));
  const { communityId } = await context.params;

  try {
    const surveyId = await createHoaSurvey(gate.user.id, communityId, parsed.data);
    return NextResponse.json({ surveyId }, { status: 201 });
  } catch (error) {
    return hoaErrorResponse("survey creation", error);
  }
}
