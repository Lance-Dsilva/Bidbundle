...
export default async function HoaInvitationPage({ params }: PageProps) {
  ...
  return (
    <div className="signup-layout bg-white">
      <AuthShowcase
        testimonial={{
          quote: "One verified invitation connects you to your HOA's services, requests, and monthly surveys.",
      ...
      />
      <main
        className="relative flex min-w-0 items-center justify-center overflow-y-auto px-5 py-10 lg:px-12"
        ...
        {usable ? (
          <div className="w-full max-w-md">
            <p className="mb-3 text-center text-[11px] font-bold uppercase tracking-[0.16em] text-[#0f8f83]">
              {invitation.role === "hoa_manager" ? "HOA manager invitation" : "HOA resident invitation"}
            </p>
            <SignUp
              path={`/hoa/join/${invitationId}`}
              routing="path"
              signInUrl={`/hoa/invitation-sign-in/${invitationId}`}
